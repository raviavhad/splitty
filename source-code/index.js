...
const CKB_URL = process.env.CKB_URL || req.query.url // example node url 'http://localhost:8114'
...
const bootstrap = async () => {
  const privateKey = process.env.PRIV_KEY || req.qurey.private_key // example private key '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee'
   ...
   * send transaction
   */
  const toAddress = ckb.utils.privateKeyToAddress(req.query.to_addr, {
    prefix: 'ckt'
  })
//"0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"
  ...
  const rawTransaction = ckb.generateRawTransaction({
    ...
    toAddress,
    capacity: BigInt(req.query.amt),//600000000000
    fee: BigInt(100000),
    safeMode: true,
    cells: unspentCells,
  ...
  res.send(`The real transaction hash is: ${realTxHash}`);
}
